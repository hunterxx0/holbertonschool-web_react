...
describe('App context state ', () => {
  test('log in and logout state check', () => {
    const wrapper = mount(
  ...
  test('markNotificationAsRead check', () => {
    const wrapper = mount(
      <AppContext.Provider value={{ user, logOut }}>
        <App/>
      </AppContext.Provider>
      );
    expect(wrapper.state().listNotifications.length).to.equals(3);
    /*wrapper.instance().markNotificationAsRead(1);
    expect(wrapper.state().listNotifications.length).to.equals(2);
*/  });
